...
# ============================================================
#
# The learner uses 16 market-state fields.
#
# Raw numeric features are converted into buckets so that
# small numerical differences do not create separate patterns.
#
# IMPORTANT:
# No Moving Average fields are used.
# ============================================================
...
class LearningEngine:
    ...
    def __init__(
        ...
        if train_ratio <= 0:
            raise ValueError(
                "Train ratio must be > 0."
            )

        if validation_ratio <= 0:
            raise ValueError(
                "Validation ratio must be > 0."
            )
    ...
    def split(
        ...
        validation_end = (
            train_end
            + int(total * self.validation_ratio)
        )
    ...
    def train(
        ...
        data = self._prepare_learning_states(
            train_data
        )
        ...
        model = {
            ...
            "model_type":
                "bucketed_historical_state_pattern",
    ...
    def _prepare_learning_states(
        ...
        prepared["move_3_bucket"] = (
            prepared["move_3_pct"]
            .apply(self._bucket_move)
        )

        prepared["move_5_bucket"] = (
            prepared["move_5_pct"]
            .apply(self._bucket_move)
        )

        prepared["range_5_bucket"] = (
            prepared["range_5_pct"]
            .apply(self._bucket_range)
        )

        prepared["candle_body_bucket"] = (
            prepared["candle_body_pct"]
            .apply(self._bucket_body)
        )

        prepared["pullback_bucket"] = (
            prepared[
                "pullback_from_high_pct"
            ]
            .apply(self._bucket_pullback)
        )

        prepared["recovery_bucket"] = (
            prepared[
                "recovery_from_low_pct"
            ]
            .apply(self._bucket_recovery)
        )
        ...
        prepared["rsi_change_3_bucket"] = (
            prepared["rsi_change_3"]
            .apply(self._bucket_rsi_change)
        )
    ...
    def _validate_dataset(
        ...
        if missing:
            ...
            raise ValueError(
                "Learning dataset missing columns: "
                f"{sorted(missing)}"
            )
